...
model_matrix = function(dat, dat.baseline, dat.left){
  X = as.matrix(dat.baseline[,c(5,6)])
  Z = as.matrix(dat[,c(7)]) #CHANGE THIS LATER
  X_long = as.matrix(dat[,c(5,6)])
  Z_last = Z[which(dat$last_record ==1),]
  rep = dat.baseline$rep
  last_rec = dat$last_record
  left_pos = dat$left_position
  left_z = as.matrix(dat.left[,c(7)]) #CHANGE THIS LATER
  left_x = as.matrix(dat.left[,c(5,6)])
